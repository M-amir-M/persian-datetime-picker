...
/// See also:
///
///  * [PCupertinoTimerPicker], the class that implements the iOS-style timer picker.
///  * [PCupertinoPicker], the class that implements a content agnostic spinner UI.
  ...
  State<StatefulWidget> createState() {
    ...
    switch (mode) {
      ...
      case PCupertinoDatePickerMode.dateAndTime:
        return _PCupertinoDatePickerDateTimeState();
      case PCupertinoDatePickerMode.date:
        return _PCupertinoDatePickerDateState();
    }
  ...
  static double _getColumnWidth(
    ...
    switch (columnType) {
      case _PickerColumnType.date:
        ...
        for (int i = 1; i <= 12; i++) {
          // An arbitrary date.
          final String date = Jalali(1400, i, 25).formatMediumDate();
...
class _PCupertinoDatePickerDateTimeState extends State<PCupertinoDatePicker> {
...
class _PCupertinoDatePickerDateState extends State<PCupertinoDatePicker> {
  int textDirectionFactor;
  CupertinoLocalizations localizations;
...
/// Different modes of [PCupertinoTimerPicker].
///
/// See also:
///
///  * [PCupertinoTimerPicker], the class that implements the iOS-style timer picker.
///  * [PCupertinoPicker], the class that implements a content agnostic spinner UI.
enum PCupertinoTimerPickerMode {
...
///
/// There are several modes of the timer picker listed in [PCupertinoTimerPickerMode].
///
/// The picker has a fixed size of 320 x 216, in logical pixels, with the exception
/// of [PCupertinoTimerPickerMode.hms], which is 330 x 216. If the parent widget
...
///    of the iOS-style date picker.
///  * [PCupertinoPicker], the class that implements a content agnostic spinner UI.
class PCupertinoTimerPicker extends StatefulWidget {
  /// Constructs an iOS style countdown timer picker.
  ///
  /// [mode] is one of the modes listed in [PCupertinoTimerPickerMode] and
  /// defaults to [PCupertinoTimerPickerMode.hms].
  ///
  ...
  /// positive integer factor of 60.
  PCupertinoTimerPicker({
    Key key,
    this.mode = PCupertinoTimerPickerMode.hms,
  ...
  final PCupertinoTimerPickerMode mode;
...
class _CupertinoTimerPickerState extends State<PCupertinoTimerPicker> {
  ...
  void initState() {
    ...
    if (widget.mode != PCupertinoTimerPickerMode.ms)
      selectedHour = widget.initialTimerDuration.inHours;

    if (widget.mode != PCupertinoTimerPickerMode.hm)
  ...
  @override
  void didUpdateWidget(PCupertinoTimerPicker oldWidget) {
    super.didUpdateWidget(oldWidget);

    assert(
      oldWidget.mode == widget.mode,
      "The PCupertinoTimerPicker's mode cannot change once it's built",
    );
  }
  ...
  Widget _buildMinutePicker(EdgeInsetsDirectional additionalPadding) {
    ...
    switch (widget.mode) {
      case PCupertinoTimerPickerMode.hm:
        offAxisFraction = 0.5 * textDirectionFactor;
        break;
      case PCupertinoTimerPickerMode.hms:
        offAxisFraction = 0.0;
        break;
      case PCupertinoTimerPickerMode.ms:
        offAxisFraction = -0.5 * textDirectionFactor;
    }
